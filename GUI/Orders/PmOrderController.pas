...
begin
  Result :=
    Order.OrderItems.ApplyUpdates  // обновление состава заказа - таблица итемов
...
begin
  // Здесь нельзя заменять на RefreshData.
  // Ломается фильтр, например, по заказчику. 
...
var r: integer;
begin
  r := RusMessageDlg('Заказ был изменен. Сохранить изменения?', mtConfirmation, [mbYes, mbNo, mbCancel], 0);
  if r = mrYes then
    Result := SaveAndClose
  else if r = mrNo then
  begin
